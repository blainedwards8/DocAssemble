...
const StructureEditor = ({ pb, initialStructure, snippets, onUpdateSnippets, onSave, onBack, activeDocumentId, variables, slotValues, tierStyles, continuousNumbering, onContentChange, onTitleChange }) => {
    ...
    const filteredProvisions = useMemo(() => {
        ...
        // 1. Filter by category if a section is selected
        if (selectedSection) {
            const category = selectedSection.category || 'Uncategorized';
            result = result.filter(s => (s.category || 'Uncategorized') === category);
        }
        ...
        return result;
    }, [snippets, selectedSection, provisionSearch]);
...
Normal # Markdown Headers are supported."
                ...
                <div className={`flex bg-slate-50 overflow-hidden shrink-0 transition-all duration-300 ${selectedSection ? 'w-[640px]' : 'w-80'}`}>
                        ...
                        <div>
                            <div className="flex items-center justify-between mb-4">
                                ...
                                <button
                                    ...
                                    className={`text-[10px] font-black uppercase px-2 py-1 rounded-md transition-all ${!selectedSection ? 'bg-indigo-600 text-white' : 'bg-slate-100 text-slate-400 hover:bg-slate-200 hover:text-slate-600'}`}
                                >
                                    Library
                                </button>
                            </div>
                    ...
                    {/* Column 2: Provisions (Always Visible, Contextual) */}
                    <div className="w-80 bg-white p-6 overflow-y-auto custom-scrollbar flex flex-col border-r border-slate-200 animate-in slide-in-from-right duration-300">
                        <div className="flex items-center justify-between mb-2">
                            <div>
                                <h3 className="text-xs font-black uppercase tracking-widest text-slate-400 mb-1">
                                    {selectedSection ? 'Category Provisions' : 'All Provisions'}
                                </h3>
                                {selectedSection && (
                        ...
                        <div className="mb-4 relative">
                            <input
                                ...
                                className="w-full pl-8 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-lg text-xs focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 outline-none transition-all"
                            />
                            <Icon name="Search" size={12} className="absolute left-2.5 top-2.5 text-slate-400" />
                            {provisionSearch && (
                                <button
                                    onClick={() => setProvisionSearch('')}
                        ...
                        <div className="space-y-3">
                            {filteredProvisions.length > 0 ? (
                                filteredProvisions.map(p => (
                                    <div
                                        key={p.id}
                                        onClick={() => { setEditingProvision(p); setIsProvisionModalOpen(true); }}
                                        className="group p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:border-indigo-200 hover:shadow-md transition-all cursor-pointer relative"
                                    >
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="flex-1">
                                                <h4 className="text-[11px] font-black text-slate-800 line-clamp-2 leading-tight group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{p.title}</h4>
                                                {p.tag && (
                                            ...
                                            <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); setEditingProvision(p); setIsProvisionModalOpen(true); }}
                                                ...
                                                <button
                                                    onClick={(e) => { e.stopPropagation(); handleDeleteProvision(p.id); }}
                                ...
                                <div className="text-center py-10 bg-slate-50/50 rounded-xl border border-dashed border-slate-200">
                                    <p className="text-[10px] font-black uppercase tracking-widest text-slate-300">
                                        {provisionSearch ? 'No matches found' : 'No Provisions'}
                                    </p>
